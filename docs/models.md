...
Web-прототип позволяет выбрать любую из них на лету через селектор в UI.
Backend читает соответствующий чекпоинт из `results/saved_models/` (см.
[architecture.md](architecture.md#backendappmodel_loaderpy)) и кеширует
загруженные модели через `lru_cache(maxsize=4)`. Если файла
`best_<model>.pth` нет — пункт в селекторе помечается как `(missing)` и
